    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          const double delta_x = circle_center_x - actor_center_x;
          const double delta_y = circle_center_y - actor_center_y;
          const double dist_sq = (delta_x * delta_x) + (delta_y * delta_y);
          const double radius_sum = CIRCLE_RADII[c] + CIRCLE_RADII[c2];

          collision = (dist_sq < (radius_sum * radius_sum));
        }
      }
    }
  }
